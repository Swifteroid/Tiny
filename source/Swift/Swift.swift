...
extension String {
    public func lowercasedFirst() -> String { self.isEmpty ? self : self[...self.startIndex].lowercased() + self[self.index(after: self.startIndex)...] }
    public func uppercasedFirst() -> String { self.isEmpty ? self : self[...self.startIndex].uppercased() + self[self.index(after: self.startIndex)...] }
}
...
extension Dictionary {
    public static func + (lhs: Dictionary, rhs: Dictionary) -> Dictionary {
        rhs.reduce(into: lhs, { $0[$1.key] = $1.value })
    }
}
...
extension Array where Element: Equatable {
    @discardableResult public mutating func remove(element: Element, first: Bool = false) -> [Element] {
        self.remove(elements: [element], first: first)
    }
...
extension Array {
    ...
    @discardableResult public mutating func removeFirst(where predicate: (Element) -> Bool) -> Element? { nil }
}
